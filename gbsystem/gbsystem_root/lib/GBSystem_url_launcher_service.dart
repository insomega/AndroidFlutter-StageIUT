...
class GBSystem_UrlLauncherService {
  void callNumber(BuildContext context, {required String number}) async {
    final Uri phoneUri = Uri(scheme: 'tel', path: number);
    try {
      await launchUrl(phoneUri);
    } catch (e) {
      showErrorDialog("Impossible de lancer cette appel");
    }
  }

  void openWebsite({required String webSite}) async {
  ...
  void sendEmail({required String emailReceiver, required String subject, required String msg}) async {
  ...
  void openYouTubeVideo({required String youtubeUrlCode}) async {
    final Uri youtubeUri = Uri.parse('https://www.youtube.com/watch?v=$youtubeUrlCode');
  ...
  void openWhatsApp({required String phoneNumber, required String msg}) async {
    final Uri whatsappUri = Uri.parse('whatsapp://send?phone=$phoneNumber&text=$msg');
