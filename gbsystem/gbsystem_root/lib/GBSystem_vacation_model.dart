...
//  compare with lie_idf , to use set()
  ...
  static GBSystem_Vacation_Model fromJson(json) {
    DateTime? pntgsStartHourIn, pntgsStartHourOut, vacStartHour, vacEndHour, vacBreakstartHour, vacBreakendHour;
    String? vacBreak;

    if ((json["PNTGS_START_HOUR_IN"] != null) && (json["PNTGS_START_HOUR_IN"].toString().isNotEmpty)) {
      pntgsStartHourIn = DateFormat(detecteDateFormat(json["PNTGS_START_HOUR_IN"])).parse(json["PNTGS_START_HOUR_IN"]);
    }

    if ((json["PNTGS_START_HOUR_OUT"] != null) && (json["PNTGS_START_HOUR_OUT"].toString().isNotEmpty)) {
      pntgsStartHourOut = DateFormat(detecteDateFormat(json["PNTGS_START_HOUR_OUT"])).parse(json["PNTGS_START_HOUR_OUT"]);
    }

    if ((json["VAC_START_HOUR"] != null) && (json["VAC_START_HOUR"].toString().isNotEmpty)) {
      vacStartHour = DateFormat(detecteDateFormat(json["VAC_START_HOUR"])).parse(json["VAC_START_HOUR"]);
    }

    if ((json["VAC_END_HOUR"] != null) && (json["VAC_END_HOUR"].toString().isNotEmpty)) {
      vacEndHour = DateFormat(detecteDateFormat(json["VAC_END_HOUR"])).parse(json["VAC_END_HOUR"]);
    }

    if ((json["VAC_BREAKSTART_HOUR"] != null) && (json["VAC_BREAKSTART_HOUR"].toString().isNotEmpty)) {
      vacBreakstartHour = DateFormat(detecteDateFormat(json["VAC_BREAKSTART_HOUR"])).parse(json["VAC_BREAKSTART_HOUR"]);
    }

    if ((json["VAC_BREAKEND_HOUR"] != null) && (json["VAC_BREAKEND_HOUR"].toString().isNotEmpty)) {
      vacBreakendHour = DateFormat(detecteDateFormat(json["VAC_BREAKEND_HOUR"])).parse(json["VAC_BREAKEND_HOUR"]);
    }
    if ((json["VAC_BREAK"] != null) && (json["VAC_BREAK"].toString().isNotEmpty)) {
      vacBreak = json["VAC_BREAK"];
    }

    TimeOfDay heureDebut = TimeOfDay.fromDateTime(vacStartHour as DateTime);
    TimeOfDay heureFin = TimeOfDay.fromDateTime(vacEndHour as DateTime);

    if ((vacBreakstartHour != null) & (vacBreakstartHour != null)) {
      TimeOfDay heuredebutPause = TimeOfDay.fromDateTime(vacBreakstartHour as DateTime);
      TimeOfDay heurefinPause = TimeOfDay.fromDateTime(vacBreakendHour as DateTime);

      final hourD = heuredebutPause.hour.toString().padLeft(2, '0');
      final minuteD = heuredebutPause.minute.toString().padLeft(2, '0');
      final hourF = heurefinPause.hour.toString().padLeft(2, '0');
      final minuteF = heurefinPause.minute.toString().padLeft(2, '0');

      vacBreak = "$hourD:$minuteD $hourF:$minuteF";
    }
    ...
    return GBSystem_Vacation_Model(
      ...
      HEURE_FIN: heureFin,
      VAC_START_HOUR: vacStartHour,
      VAC_END_HOUR: vacEndHour,
      ...
      PNTGS_START_HOUR_IN: pntgsStartHourIn,
      PNTGS_START_HOUR_OUT: pntgsStartHourOut,
      PNTG_END_HOUR: json["PNTG_END_HOUR"],
      PNTG_START_HOUR: json["PNTG_START_HOUR"],
      VAC_BREAK: vacBreak, //json["VAC_BREAK"],
