...
import '../../components/time_detail_card.dart'; // Importe le widget [TimeDetailCard] personnalisé.
import '../../core/service.dart'; // Importe le modèle de données [Service].
...
/// le filtrage par date et recherche, et l'import/export de données Excel.
class PriseServiceScreen extends StatefulWidget {
  /// Constructeur de [PriseServiceScreen].
...
/// La classe d'état associée à [PriseServiceScreen].
///
/// Elle gère l'état mutable du widget, y compris les données des services,
/// les filtres de date, les requêtes de recherche et les contrôleurs de défilement.
class _PriseServiceScreenState extends State<PriseServiceScreen> {
  /// Date et heure actuellement affichées dans le pied de page (footer).
  DateTime _currentDisplayDate = DateTime.now();

  /// Date de début utilisée pour filtrer les services.
  DateTime _startDate = DateTime(2025, 7, 1);

  /// Date de fin utilisée pour filtrer les services.
  DateTime _endDate = DateTime(2025, 7, 31);

  /// [Timer] utilisé pour mettre à jour l'heure affichée chaque seconde.
  Timer? _timer;

  /// [ScrollController] pour la colonne des services de type 'début'.
  final ScrollController _debutScrollController = ScrollController();

  /// [ScrollController] pour la colonne des services de type 'fin'.
  final ScrollController _finScrollController = ScrollController();

  /// [ScrollController] pour la colonne des services de type 'résultat'.
  final ScrollController _resultatScrollController = ScrollController();

  /// [TextEditingController] pour le champ de recherche.
  final TextEditingController _searchController = TextEditingController();

  /// La chaîne de caractères saisie dans la barre de recherche.
  String _searchQuery = '';

  /// Liste principale de tous les services chargés, généralement depuis un fichier Excel.
  List<Service> _services = [];

  /// Indicateur pour savoir si des données ont été chargées depuis un fichier Excel.
  bool _dataLoaded = false;

  /// Indicateur de visibilité pour la colonne 'Début'.
  bool _showDebutColumn = true;

  /// Indicateur de visibilité pour la colonne 'Fin'.
  bool _showFinColumn = true;

  /// Indicateur de visibilité pour la colonne 'Résultat'.
  ...
  void initState() {
    ...
    /// Définit la locale par défaut sur le français pour le formatage des dates.
    Intl.defaultLocale = 'fr_FR';
  }

  /// Met à jour la chaîne de recherche lorsque le texte change dans le champ,
  ...
  @override
  void dispose() {
    /// Annule le [Timer] pour éviter les fuites de mémoire lorsque le widget est supprimé.
    _timer?.cancel();
    /// Libère les [ScrollController] et [TextEditingController].
  ...
  /// Change la date spécifiée ([dateToChange]) d'un certain nombre de jours.
  ///
  /// [dateToChange]: La date à modifier (soit [_startDate], soit [_endDate]).
  /// [daysToAdd]: Le nombre de jours à ajouter ou soustraire.
  /// [updateState]: Fonction de rappel pour mettre à jour la date dans l'état du widget.
  ...
  /// Change la date spécifiée ([dateToChange]) d'un certain nombre de mois.
  ///
  /// [dateToChange]: La date à modifier (soit [_startDate], soit [_endDate]).
  ...
  ///
  /// [context]: Le [BuildContext] du widget.
  /// [initialDate]: La date par défaut affichée dans le sélecteur.
  /// [updateState]: Fonction de rappel pour mettre à jour la date dans l'état du widget.
  Future<void> _selectDate(BuildContext context, DateTime initialDate, Function(DateTime) updateState) async {
    ...
    if (picked != null && picked != initialDate) {
      // Si une date est sélectionnée et qu'elle est différente de l'initiale.
      setState(() {
  ...
  /// C'est une manière implicite de rafraîchir l'interface utilisateur basée sur des données dérivées.
  ...
  /// Getter pour obtenir les services filtrés par date et recherche,
  ...
  List<Service> get _filteredAndSortedDebutServices {
    final filteredList = _services.where((service) {
      ...
      bool isInDateRange = service.startTime.isBefore(_endDate.endOfDay()) &&
          service.startTime.isAfter(_startDate.startOfDay());
  ...
  List<Service> get _filteredAndSortedFinServices {
    final filteredList = _services.where((service) {
      ...
      bool isInDateRange = service.endTime.isBefore(_endDate.endOfDay()) &&
          service.endTime.isAfter(_startDate.startOfDay());
  ...
  /// [newAbsentStatus]: Le nouveau statut d'absence à appliquer (true pour absent, false pour présent).
      ...
      if (serviceIndex != -1) {
        ...
        if (newAbsentStatus == true) {
          ...
          _services[serviceIndex] =
              _services[serviceIndex].copyWith(isAbsent: newAbsentStatus, isValidated: false);
  ...
  ///
  /// [serviceId]: L'identifiant unique du service à modifier.
  /// [newValidateStatus]: Le nouveau statut de validation à appliquer (true pour validé, false pour non validé).
  ...
  ///
  /// Ouvre un sélecteur de date puis un sélecteur d'heure pour permettre à l'utilisateur
  /// de choisir une nouvelle date et heure pour le service. Effectue des validations
  /// pour s'assurer que l'heure de début reste antérieure à l'heure de fin, et vice-versa.
  ///
  /// [serviceId]: L'identifiant unique du service à modifier.
  /// [currentTime]: L'heure actuelle du service (début ou fin) avant la modification.
  /// [type]: Le type de carte ([TimeCardType.debut] ou [TimeCardType.fin]) indiquant quelle heure est modifiée.
  Future<void> _handleModifyTime(String serviceId, DateTime currentTime, TimeCardType type) async {
    ...
    if (newTime != null) {
      ...
      if (type == TimeCardType.debut) {
        ...
        if (updatedDateTime.isAfter(currentService.endTime) ||
            updatedDateTime.isAtSameMomentAs(currentService.endTime)) {
          canUpdate = false;
          errorMessage =
              'L\'heure de début (${DateFormat('dd/MM HH:mm').format(updatedDateTime)}) ne peut pas être après ou égale à l\'heure de fin actuelle (${DateFormat('dd/MM HH:mm').format(currentService.endTime)}).';
        }
      } else {
        ...
        if (updatedDateTime.isBefore(currentService.startTime) ||
            updatedDateTime.isAtSameMomentAs(currentService.startTime)) {
          canUpdate = false;
          errorMessage =
              'L\'heure de fin (${DateFormat('dd/MM HH:mm').format(updatedDateTime)}) ne peut pas être avant ou égale à l\'heure de début actuelle (${DateFormat('dd/MM HH:mm').format(currentService.startTime)}).';
        }
      }
      ...
      if (canUpdate) {
        setState(() {
          ...
          if (type == TimeCardType.debut) {
            ...
            debugPrint(
                'Service $serviceId - Nouvelle heure de début: ${DateFormat('dd/MM HH:mm').format(updatedDateTime)}');
          } else {
            _services[serviceIndex] = currentService.copyWith(endTime: updatedDateTime);
            debugPrint(
                'Service $serviceId - Nouvelle heure de fin: ${DateFormat('dd/MM HH:mm').format(updatedDateTime)}');
          }
        });
  ...
  ///
  /// [serviceToScrollTo]: Le [Service] vers lequel faire défiler l'affichage.
  void _scrollToService(Service serviceToScrollTo) {
    /// Utilisez les listes filtrées qui sont effectivement affichées dans les [ListView].
    ...
    if (serviceIndex != -1) {
      // Si le service est trouvé dans la liste.
  ...
  ///
  /// Ouvre un sélecteur de fichiers, lit le fichier Excel (.xlsx) choisi,
  /// et parse les données pour créer une liste de [Service] objets.
  /// Met à jour l'état du widget avec les services importés.
    ...
    try {
      ...
      if (result != null && result.files.single.path != null) {
        ...
        if (sheet == null || sheet.rows.isEmpty) {
          // Vérifie si la feuille est vide.
  ...
  ///
  /// Crée un fichier Excel (.xlsx) à partir de la liste des services fournis,
  /// puis permet à l'utilisateur de choisir un répertoire de sauvegarde
  /// et ouvre le fichier une fois l'exportation terminée.
  ///
  /// [servicesToExport]: La liste des [Service] objets à exporter.
  Future<void> _exportServicesToExcel(List<Service> servicesToExport) async {
    ...
    try {
      ...
      final String? directoryPath =
          await FilePicker.platform.getDirectoryPath(); // Ouvre une fenêtre de dialogue pour que l'utilisateur choisisse un répertoire.
...
/// Méthode pour construire l'AppBar de l'application.
 ///
 /// L'AppBar s'adapte à l'orientation de l'appareil (portrait ou paysage)
 /// pour afficher différemment les éléments (titre, logo, boutons d'action).
 ///
 /// En mode portrait, les boutons d'action sont placés dans un `bottom` séparé
 /// sous le titre principal. En mode paysage, les actions sont affichées
 /// à droite du titre.
 ///
 /// [context] Le BuildContext de l'application.
 /// Retourne un widget [AppBar] configuré pour l'orientation actuelle.
 AppBar _buildAppBar(BuildContext context) {
   ...
   if (orientation == Orientation.portrait) {
     ...
     return AppBar(
       ...
       /// Hauteur standard pour le titre et le logo.
       toolbarHeight: responsive_utils.responsivePadding(context, 80.0),
       /// Widget affiché au début de l'AppBar, généralement le logo de l'application.
       leading: Padding(
       ...
       bottom: PreferredSize(
         ...
         /// Hauteur pour les boutons d'action.
     ...
     return AppBar(
       ...
       backgroundColor: Theme.of(context).primaryColor,
       foregroundColor: Theme.of(context).colorScheme.onPrimary,
       /// Widgets d'action affichés à droite de l'AppBar en mode paysage.
       actions: [
         Padding(
           padding: EdgeInsets.symmetric(horizontal: responsive_utils.responsivePadding(context, 2.0)),
           /// Affiche un bouton différent selon si les données sont déjà chargées ou non.
 ...
 /// Méthode pour construire le sélecteur de plage de dates et les boutons
 /// de visibilité des colonnes (D, F, R pour Début, Fin, Résultat).
 ///
 /// La mise en page s'adapte à l'orientation de l'appareil. En mode portrait,
 /// les contrôles de date sont au-dessus des boutons de colonne et de la date
 /// actuelle. En mode paysage, tous les éléments sont alignés horizontalement.
 ///
 /// Retourne un widget [Container] contenant le sélecteur de date et les boutons.
 Widget _buildDateRangeSelector() {
   ...
   /// [onChanged] Un rappel qui est appelé lorsque la visibilité est modifiée.
 ...
 ///
 /// Permet d'incrémenter ou de décrémenter un jour, ou d'ouvrir un sélecteur de date.
 ///
 /// [date] La date à afficher et à modifier.
 /// [onDateChanged] Un rappel qui est appelé lorsque la date est modifiée.
 /// Retourne un widget [Row] contenant les contrôles de date.
 ...
 ///
 /// Permet à l'utilisateur de rechercher des services par nom d'employé.
 ///
 /// Retourne un widget [Padding] contenant un [TextField] pour la recherche.
 ...
 /// Méthode pour construire les en-têtes de colonne de la liste des services.
 ///
 /// Les en-têtes "Début", "Fin" et "Résultat" sont affichés conditionnellement
 /// en fonction des variables de visibilité (`_showDebutColumn`, `_showFinColumn`, `_showResultColumn`).
 ///
 /// Retourne un widget [Padding] contenant une [Row] avec les en-têtes.
 ...
 /// Méthode pour construire une liste de services sous forme de colonne.
 ///
 /// Cette méthode crée une [Expanded] qui contient une [ListView.builder]
 /// pour afficher les cartes de service. La flexibilité de la colonne
 /// varie en fonction du [type] de carte (début, fin, ou résultat).
 ///
 /// [services] La liste des objets [Service] à afficher.
 /// [controller] Le [ScrollController] pour cette liste, permettant la synchronisation du défilement.
 /// [type] Le type de carte de temps ([TimeCardType.debut], [TimeCardType.fin], ou [TimeCardType.result]).
 /// Retourne un widget [Expanded] contenant la liste des services.
 ...
 ///
 /// Affiche une information de copyright et la date/heure actuelle.
 ///
 ...
 Widget _buildFooter() {
  ...
  /// Construit l'interface utilisateur principale de l'écran.
 ///
 /// Cette méthode est le point d'entrée pour la construction du widget tree de cet écran.
 /// Elle assemble les différentes parties de l'UI : l'AppBar, le sélecteur de date,
 /// la barre de recherche, les en-têtes de colonne, les listes de services (conditionnelles)
 /// et le pied de page.
 ///
 /// Un message est affiché si aucune donnée n'est chargée, incitant l'utilisateur
 /// à importer un fichier Excel.
 ///
 /// [context] Le BuildContext de l'application.
 /// Retourne un widget [Scaffold] contenant toute l'interface.
  @override
