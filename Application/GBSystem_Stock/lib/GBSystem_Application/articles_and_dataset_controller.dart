...
import 'package:gbsystem_stock/GBSystem_Application/GBSystem_articles_and_dataset_model.dart';
...
class ArticlesAndDatasetController extends GetxController {
  Rx<ArticleAndDataSetModel?>? _currentArticlesAndDataSet = Rx<ArticleAndDataSetModel?>(null);
  ...
  set setCurrentArticles(List<ArticleSalarieGestionStockModel> articlesSalarie) {
  ...
  set setArticleToRight(ArticleSalarieGestionStockModel newArticle) {
    _currentArticlesAndDataSet?.value?.listArticles.insert(_currentArticlesAndDataSet?.value?.listArticles.length ?? 0, newArticle);
    update();
  }
  ...
  List<ArticleSalarieGestionStockModel>? get getAllArticles => _currentArticlesAndDataSet?.value?.listArticles;

  Rx<List<ArticleSalarieGestionStockModel>?> get getAllArticlesRx => Rx(_currentArticlesAndDataSet?.value?.listArticles);

  ArticleAndDataSetModel? get getCurrentArticlesAndDataSet => _currentArticlesAndDataSet?.value;
  SalarieDataSetGestionStockModel? get getCurrentDataSet => _currentArticlesAndDataSet?.value?.salarieDataSetModel;

  Rx<ArticleAndDataSetModel?>? get getCurrentArticlesAndDataSetRx => _currentArticlesAndDataSet;
}
