...
class GBSystemSelectItemColorScreenController extends GetxController {
  GBSystemSelectItemColorScreenController({required this.context});
  ...
  void filterDataSite(String query) {
    ...
    filtredColors.value = colors.where((color) {
      return color.CLR_LIB.toString().toLowerCase().contains(query.toLowerCase()) || color.CLRCAT_LIB.toString().toLowerCase().contains(query.toLowerCase());
    }).toList();
  }

  void selectItemSiteFunction({required GbsystemColorModel selectedColor}) async {
