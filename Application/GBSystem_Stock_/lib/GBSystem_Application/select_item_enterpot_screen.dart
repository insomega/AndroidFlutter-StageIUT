...
import 'package:gbsystem_stock/GBSystem_Application/select_item_enterpot_screen_controller.dart';
import 'package:gbsystem_translations/GBSystem_Application_Strings.dart';

import 'package:get/get.dart';

class GBSystem_SelectItemEnterpotScreen extends StatelessWidget {
  const GBSystem_SelectItemEnterpotScreen({super.key, required this.controllerTxtField});

  final TextEditingController controllerTxtField;
  @override
  Widget build(BuildContext context) {
    final GBSystemSelectItemEnterpotScreenController m = Get.put(GBSystemSelectItemEnterpotScreenController(context: context));
                                  ...
                                  itemBuilder: (context, index) {
                                    return Padding(
                                      ...
                                      child: GBSystem_Root_CardViewWidgetGenerique(
                                        ...
                                        onTap: () {
                                          m.selectItemSiteFunction(selectedEntr: m.filtredEnterpot[index]);

                                          controllerTxtField.text = m.filtredEnterpot[index].ENTR_LIB;
                                        },
                                        title: m.filtredEnterpot[index].ENTR_LIB,
                                        optStr1: m.filtredEnterpot[index].ENTR_CODE,
                                        optStr2: m.filtredEnterpot[index].ENTR_TELPH,
                                        optStr3: m.filtredEnterpot[index].VIL_LIB,
                            ...
                            itemBuilder: (context, index) {
                              return Padding(
                                ...
                                child: GBSystem_Root_CardViewWidgetGenerique(
                                  tileColor: index % 2 == 0 ? Colors.grey.withOpacity(0.2) : Colors.white,
                                  onTap: () {
                                    m.selectItemSiteFunction(selectedEntr: m.enterpot[index]);
                                    controllerTxtField.text = m.enterpot[index].ENTR_LIB;
