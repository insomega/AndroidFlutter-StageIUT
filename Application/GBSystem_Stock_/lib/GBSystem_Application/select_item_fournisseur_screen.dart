...
import 'package:gbsystem_stock/GBSystem_Application/select_item_fournisseur_screen_controller.dart';
import 'package:gbsystem_translations/GBSystem_Application_Strings.dart';
import 'package:get/get.dart';

class GBSystem_SelectItemFournisseurScreen extends StatelessWidget {
  const GBSystem_SelectItemFournisseurScreen({super.key, required this.controllerTxtField});

  final TextEditingController controllerTxtField;
  @override
  Widget build(BuildContext context) {
    final GBSystemSelectItemFournisseurScreenController m = Get.put(GBSystemSelectItemFournisseurScreenController(context: context));
                                ...
                                child: ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    return GBSystem_Root_CardViewWidgetGenerique(
                                      ...
                                      onTap: () {
                                        m.selectItemSiteFunction(selectedFourni: m.filtredFournisseur[index]);

                                        controllerTxtField.text = m.filtredFournisseur[index].FOU_LIB;
                                      },
                                      title: m.filtredFournisseur[index].FOU_LIB,
                                      optStr1: m.filtredFournisseur[index].FOU_TEL,
                                      optStr2: m.filtredFournisseur[index].FOU_ADR1,
                                      optStr3: m.filtredFournisseur[index].FOU_TELPOR,
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              return GBSystem_Root_CardViewWidgetGenerique(
                                tileColor: index % 2 == 0 ? Colors.grey.withOpacity(0.2) : Colors.white,
                                onTap: () {
                                  m.selectItemSiteFunction(selectedFourni: m.fournisseur[index]);
                                  controllerTxtField.text = m.fournisseur[index].FOU_LIB;
