...
import 'choose_mode_stock_screen_controller.dart';
import 'home_add_article_screen.dart';
import 'GBSystem_home_gestion_stock_screen.dart';
import 'select_item_article_screen.dart';
import 'GBSystem_auth_service.dart';
import 'choose_mode_button_widget.dart';
import 'package:get/get.dart';
import 'package:local_auth/local_auth.dart';
import 'GBSystem_home_affectuer_articles_gestion_stock_screen.dart';
...
class _ChooseModeStockScreenState extends State<ChooseModeStockScreen> {
  ...
  Widget build(BuildContext context) {
    final ChooseModeStockScreenController m = Get.put(ChooseModeStockScreenController(context: context));
    return WillPopScope(
      onWillPop: () {
        exit(0);
      },
                      ...
                      ChooseModeButtonWidget(
                        onTap: () async {
                          try {
                            ...
                            m.isLoading.value = false;
                            GBSystem_Add_LogEvent(message: e.toString(), method: "GBSystemHomeGestionStockScreen", page: "choose_mode_stock_screen");
                          }
                        },
                      ...
                      ChooseModeButtonWidget(
                        onTap: () async {
                          try {
                            ...
                            m.isLoading.value = true;
                            await GBSystem_AuthService(context).getAllArticlesRef().then((articlesRef) async {
                              m.isLoading.value = false;

                              if (articlesRef != null) {
                                m.articlesController.setAllArticles = articlesRef;
                              }
                            });
