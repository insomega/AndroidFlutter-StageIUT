...
class GBSystemMemoQuestionController extends GetxController {
  ...
  bool checkExistsMemo({required MemoQuestionModel memo}) {
    ...
    _allMemo?.forEach((element) => print(element.LIEINSMMO_LIB));
  ...
  List<MemoQuestionModel>? getAllMemoFiltredWith_LIEINSPQU_IDF(String LIEINSPQU_IDF) {
    // List<MemoQuestionModel> firstFiltreMemo = _allMemo?.toSet().toList() ?? [];

    return (_allMemo ?? []).where((element) => element.LIEINSPQU_IDF == LIEINSPQU_IDF).toList();
  }
