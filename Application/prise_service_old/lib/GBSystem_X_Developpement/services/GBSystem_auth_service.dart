...
import 'package:gbs_new_project/GBSystem_X_Developpement/models/QUICK_ACCESS_MODELS/SERVER_MODEL/GBSystem_article_affectuier_multiple_gestion_stock_model.dart';
...
import 'package:gbs_new_project/GBSystem_X_Developpement/models/QUICK_ACCESS_MODELS/SERVER_MODEL/GBSystem_image_eval_non_terminer_model.dart';
...
import 'package:gbs_new_project/GBSystem_X_Developpement/models/QUICK_ACCESS_MODELS/SERVER_MODEL/GBSystem_pause_model.dart';
...
import 'package:gbs_new_project/GBSystem_X_Developpement/models/QUICK_ACCESS_MODELS/SERVER_MODEL/GBSystem_question_without_memo_model.dart';
...
import 'package:gbs_new_project/GBSystem_X_Developpement/models/QUICK_ACCESS_MODELS/SERVER_MODEL/GBSystem_reponse_cloture_model.dart';
...
import 'package:gbs_new_project/GBSystem_X_Developpement/models/QUICK_ACCESS_MODELS/APPLICATION_MODELS/GBSystem_salarie_photo_model.dart';
...
import 'package:gbs_new_project/GBSystem_X_Developpement/models/QUICK_ACCESS_MODELS/SERVER_MODEL/GBSystem_vacation_salarie.dart';
...
import 'package:gbs_new_project/GBSystem_X_Developpement/models/QUICK_ACCESS_MODELS/SERVER_MODEL/GBsystem_salarie_formulaire_causerie_model.dart';
import 'package:gbs_new_project/GBSystem_X_Developpement/models/QUICK_ACCESS_MODELS/SERVER_MODEL/data_server_model.dart';
...
import 'package:gbs_new_project/GBSystem_X_Developpement/services/GBSystem_location_service.dart';
import 'package:gbs_new_project/GBSystem_X_Developpement/services/api.dart';
// import 'package:gbs_new_project/GBSystem_X_Developpement/services/GBSystem_location_service.dart';
import 'package:gbs_new_project/GBSystem_X_Developpement/services/app_exceptions_manager_api.dart';
...
class GBSystem_AuthService {
  ...
  Future<void> saveCookies(String newCookies) async {
    ...
    prefs.setString(GbsSystemServerStrings.kCookies, newCookies);
  }

  // Future<String?> getS19ByUserNumber() async {
  //   String? s19;
  //   // = GbsSystemServerStrings.kMyS19Standard;
  //   int clientNumber = int.parse(GbsSystemServerStrings.kClientNumber);
  //   String filePath = GbsSystemServerStrings.kJsonApiPath;
  //   try {
  ...
  //   String url = GbsSystemServerStrings.kMyBaseUrlStandard;
  //   int clientNumber = int.parse(GbsSystemServerStrings.kClientNumber);
  //   String filePath = GbsSystemServerStrings.kJsonApiPath;
  //   try {
  //     String jsonString = await rootBundle.loadString(filePath);
  ...
  Future initApiData() async {
    ...
    _ActiveS19 = prefs.getString(GbsSystemServerStrings.kS19) ??
        GbsSystemServerStrings.kMyS19Standard;
    _Wid = prefs.getString(GbsSystemServerStrings.kToken);
  ...
  Future<ResponseModel> loginUser({required UserModel userModel}) async {
    ...
    ResponseModel data = await AppManageApi(context)
        .post(
            url: _ActiveUrl,
            data:
                ActiveApplication_Params.get_ConnexionData(userModel, _ActiveS19
                    // ,
                    // null
                    ))
        .then((value) async {
  ...
  Future<List<VacationModel>?> getInfoVacation() async {
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          ...
          // GbsSystemServerStrings.str_server_okey,
          "VAC_LOAD_ETAT": "0",
          "VAC_IDF": "378315",
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            ...
            data.data["Data"][i]["Planning_Vacations"] != null) {
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["Planning_Vacations"] != null &&
            data.data["Data"][i]["Planning_Vacations"] is List &&
            (data.data["Data"][i]["Planning_Vacations"] as List).isNotEmpty) {
          vacationModel = VacationModel.fromJson(
            data.data["Data"][i]["Planning_Vacations"][0],
          );
        }
      }
  ...
  Future<GBSystemSalarieWithPhotoModel?> getInfoSalarie() async {
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
        (data.data["Errors"] as List).isEmpty) {
      SalarieModel salarieModel = SalarieModel.fromJson(
          data.getFirstElementFromDataList(elementName: "ClientData"));
      String imageSalarie =
      ...
      await SharedPreferences.getInstance().then((value) {
        value.setString(GbsSystemServerStrings.kCookies, data.cookies!);
      });
      await saveCookies(data.cookies!);

      return GBSystemSalarieWithPhotoModel(
          salarieModel: salarieModel, imageSalarie: imageSalarie);
    } else {
      return null;
    }
  }
  ...
  Future<VacationModel?> pointageEntrerSorie({
    ...
    if (!internatController.isConnected) {
      LocalDatabaseService().saveRequestLocally(
        url: _ActiveUrl,
        cookies: _Cookies,
        data: data,
      );
    ...
    // test connexion = oui
      ...
      ResponseModel responseServer = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: data,
        cookies: _Cookies,
      );
  ...
  Future<VacationModel?> pointageEntrerSorieMultiple({
    required String Sens,
  ...
  Future<VacationSalarieModel?> pointageEntrerSorieSalarie({
    required String Sens,
    required SalariePlanningModel salarie,
    required SitePlanningModel site,
  }) async {
    await initApiData();
    final GBSystemInternatController internatController =
        Get.put(GBSystemInternatController());
    ...
    // test connexion = oui
      ...
      if ((responseServer.data["Data"] as List).isNotEmpty &&
          (responseServer.data["Errors"] as List).isEmpty) {
        VacationSalarieModel myVacation =
  ...
  Future<List<SalariePlanningModel>?> getAllEmployer() async {
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Server,Serveur_int,Select",
          "svr_type": "1",
          "pagination": "0",
          "row": "0",
          "FieldsToShow": "4",
  ...
  Future<List<VacationModel>?> getVacationPlanning({
    required SitePlanningModel sitePlanningModel,
  }) async {
    await initApiData();
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          ...
          "VAC_LOAD_ETAT": "0",
          "LIE_IDF": sitePlanningModel.LIE_IDF,
          "TPH_PSA": "1",
  ...
  Future<List<TypeQuestionnaireModel>?>
      getTypeQuestionnaireQuickAccess() async {
    await initApiData();
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {"OKey": "Lieu,LIEINSQUESTYP,select", "Wid": _Wid!},
        cookies: _Cookies);
  ...
  Future<List<QuestionnaireModel>?> getQuestionnaireQuickAccess() async {
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Lieu,LIEU_INSPECT_QUESTNRE_Web,select",
          "FLIEINSTYP_IDF": "1",
          "Wid": _Wid!
        },
        cookies: _Cookies);
  ...
  Future<List<EvaluationEnCoursModel>?> getListEvaluationEnCours() async {
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          "OKey":
              "Lieu,Load_Liste_evaluations_QA_Web,Load_Liste_evaluations_QA_Web",
          "START_DATE": ConvertDateService().parseDate(date: DateTime.now()),
          "END_DATE": ConvertDateService()
              .parseDate(date: DateTime.now().add(Duration(days: 1))),
          "SYSINSTYP_IDF": "1",
          "type": "0",
...
/////////
  Future<List<QuestionnaireModel>?> getQuestionnaireDependTypeQuickAccess(
      {required TypeQuestionnaireModel typeQuestionnaire}) async {
    await initApiData();
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Lieu,LIEU_INSPECT_QUESTNRE_Web,select",
          "LIEINSQUESTYP_IDF": typeQuestionnaire.LIEINSQUESTYP_IDF,
          "LIEINSTYP_IDF": "1",
          "Wid": _Wid!
        },
        cookies: _Cookies);
  ...
  Future<List<SiteQuickAccesModel>?> getSiteQuickAccessFiltred({
    required bool isLibre,
    required bool isPeriodique,
    required bool isUtilisateur,
  }) async {
    await initApiData();
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
...
//////
  Future<List<SalarieQuickAccessModel>?> getSalariesQuickAccess({
    required TypeQuestionnaireModel typeQuestionnaire,
    required QuestionnaireModel questionnaireModel,
    SiteQuickAccesModel? siteQuickAccesModel,
    required String type,
    String? utilisateur_LIE_IDF,
  }) async {
    await initApiData();
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          ...
          "START_DATE": utilisateur_LIE_IDF != null
              ? ConvertDateService().parseDate(date: firstDateMonth)
          ...
          "END_DATE": utilisateur_LIE_IDF != null
              ? ConvertDateService().parseDate(date: lastDateMonth)
              : ConvertDateService().parseDateAndTime(date: DateTime.now()),
          // "0${DateTime.now().day}/${DateTime.now().month}/${DateTime.now().year} ${DateTime.now().hour}:${DateTime.now().minute}",
          "EVAL_LIBRE": type == GbsSystemStrings.str_libre
              ? "true"
  ...
  Future<List<AgenceSalarieQuickAccesModel>?> getAgencesSalariesQuickAccess({
    required TypeQuestionnaireModel typeQuestionnaire,
    required QuestionnaireModel questionnaireModel,
    SiteQuickAccesModel? siteQuickAccesModel,
    required String type,
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          ...
          "LIEINSPQSNR_IDF": questionnaireModel.LIEINSPQSNR_IDF,
          "EVAL": "1",
          "ETAT": "2",
          "page": "1",
          "rows": "200",
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ClientData"] != null) {
          listAgnces = AgenceSalarieQuickAccesModel.convertDynamictoListAgences(
              data.data["Data"][0]["ClientData"]["rows"]);
        }
      }
  ...
  Future<List<SalarieQuickAccessModel>?> getSalariesQuickAccessUtilisateur({
    required TypeQuestionnaireModel typeQuestionnaire,
    required QuestionnaireModel questionnaireModel,
    SiteQuickAccesModel? siteQuickAccesModel,
    required String type,
  ...
  Future<String?> getPhotoSalarieQuickAccess({required String SVR_IDF}) async {
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Server_Photo,System_photo,Get_Default_Photo_From_Web",
          "PREFIX": "SVR",
          "ENTITY_IDF_NAME": "SVR_IDF",
          "ENTITY_IDF_VALUE": SVR_IDF,
          "Wid": _Wid!
        },
        cookies: _Cookies);
  ...
  Future<String?> getPhotoRoot({required int type, required String idf}) async {
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          "OKey": "Server_Photo,System_photo,Get_Default_Photo_From_Web",
          "PREFIX": prefix,
          "ENTITY_IDF_NAME": idf_name,
          "ENTITY_IDF_VALUE": idf,
          "Wid": _Wid!
        },
        cookies: _Cookies);
  ...
  Future<List<QuestionTypeModel>?> getFormulaireQuestionsTypeDataQuickAccess({
    ...
    try {
      ...
      GBSystem_ManageCatchErrors().catchErrors(
        context,
        message: e.toString(),
        method: "getFormulaireQuestionsTypeDataQuickAccess",
        page: "GBSystem_auth_service",
      );
    }
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          "OKey":
              "Lieu,load_lieu_evaluation_for_server,load_lieu_evaluation_for_server",
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ClientData"] != null) {
          listQustType = QuestionTypeModel.convertDynamictoListQuestionsType(
              data.data["Data"][i]["ClientData"]);
        }
        if (setEvaluation) {
          if (data.data["Data"][i] != null &&
              data.data["Data"][i] is Map &&
              data.data["Data"][i]["EvaluationStat"] != null &&
              (data.data["Data"][i]["EvaluationStat"] as List).isNotEmpty) {
            EvaluationModel evaluationModel = EvaluationModel.fromJson(
                data.data["Data"][i]["EvaluationStat"][0]);
  ...
  Future<List<QuestionModel>?> getFormulaireQuestionDataQuickAccess(
      {required QuestionTypeModel questionTypeModel}) async {
    await initApiData();
    final evaluationController = Get.put<GBSystemEvaluationSurSiteController>(
        GBSystemEvaluationSurSiteController());
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          "OKey":
              "Lieu,Get_Eval_For_Server_By_Categories,Get_Eval_For_Server_By_Categories",
          "LIEINSPCAT_IDF": questionTypeModel.LIEINSPCAT_IDF,
          "LIEINSPSVR_IDF": questionTypeModel.LIEINSPSVR_IDF,
          "LIEINSPQSNR_IDF":
              evaluationController.getSelectedQuestionnaire?.LIEINSPQSNR_IDF ??
                  "1",
          "Wid": _Wid!
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ClientData"] != null) {
          questionnaireWithoutMemoModel =
              QuestionWithoutMemoModel.convertDynamictoListQuestionsWithoutMemo(
                  data.data["Data"][i]["ClientData"]);
        }
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["LIEINSQMMO"] != null) {
          listMemo = MemoQuestionModel.convertDynamictoListMemo(
              data.data["Data"][i]["LIEINSQMMO"]);
      ...
      questionnaireWithoutMemoModel.forEach(
        (element) {
          print("-----------------------");
          ...
          questionnaireModel.add(QuestionModel(
              LIEINSQMMO: listMemo,
              questionWithoutMemoModel: element,
              nombreImages: 0));
        },
      );
      await saveCookies(data.cookies!);
  ...
  Future<List<MemoQuestionModel>?> getListMemo(
      {required QuestionTypeModel questionTypeModel}) async {
    await initApiData();
    final evaluationController = Get.put<GBSystemEvaluationSurSiteController>(
        GBSystemEvaluationSurSiteController());
  ...
  Future<List<ImageEvaluationNonTerminerModel>?> getListImagesQuestion(
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Server_Photo,,Get_Photo_From_Web",
          "PREFIX": 'LIEINSPSVRQU',
          "ENTITY_IDF_NAME": "LIEINSPSVRQU_IDF",
          "ENTITY_IDF_VALUE":
              questionModel.questionWithoutMemoModel.LIEINSPSVRQU_IDF,
          "Wid": _Wid!
        },
        cookies: _Cookies);
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ClientData"] != null) {
          listMemo = ImageEvaluationNonTerminerModel.convertDynamictoListImages(
              data.data["Data"][i]["ClientData"]);
        }
      }
  ...
  Future<ImageTypeTwoWithModel?> getImageDataForEvalNonTerminer(
      {required ImageEvaluationNonTerminerModel image}) async {
    await initApiData();

    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Server_Photo,System_photo,Get_Photo_From_Web",
          "PREFIX": 'LIEINSPSVRQU',
          "IMAGE_IDF": image.IMAGE_IDF,
          "Wid": _Wid!
        },
        cookies: _Cookies);
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["BinaryData"] != null) {
          imageData = data.data["Data"][i]["BinaryData"];
        }
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ClientData"] != null &&
            (data.data["Data"][i]["ClientData"] as List).isNotEmpty) {
          image = ImageTypeTwoModel.fromJson(
              (data.data["Data"][i]["ClientData"] as List).first);
        }
      }
  ...
  Future<List<ReponseQCMModel>?> getQCMQuestionReponsesDataQuickAccess(
      {required QuestionModel questionModel}) async {
    await initApiData();
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Lieu,Lieu,Get_SVR_Qust_QCM_Resp",
          "LIEINSPSVRQU_IDF":
              questionModel.questionWithoutMemoModel.LIEINSPSVRQU_IDF,
          "Wid": _Wid!
        },
  ...
  Future<EvaluationModel?> checkQCMQuickAccess(
      {required QuestionModel questionModel, required bool checkBool}) async {
    await initApiData();
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey":
              "Lieu,valide_eval_server_sur_lieu,valide_eval_server_sur_lieu",
          "LIEINSPSVRQU_IDF":
              questionModel.questionWithoutMemoModel.LIEINSPSVRQU_IDF,
          "LIEINSPSVRQU_UIDF":
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      EvaluationModel evaluationModel =
          EvaluationModel.fromJson(data.getFirstElementFromDataList());
  ...
  Future<String?> getQCM_LIEINSPQU_MAX({
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Lieu,Lieu,Get_Quest_MAX_Resp",
          "LIEINSPSVRQU_IDF":
              questionModel.questionWithoutMemoModel.LIEINSPSVRQU_IDF,
          "Wid": _Wid!
        },
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      String LIEINSPQU_MAX =
          data.getFirstElementFromDataList()["LIEINSPQU_MAX"];
  ...
  Future<EvaluationModel?> sendReponseQCMQuickAccess(
      {required QuestionModel questionModel,
      required ReponseQCMModel reponseQCMModel}) async {
    await initApiData();
  ...
  Future<EvaluationModel?> sendReponseQustionsNonApplicableBool({
    required QuestionModel questionModel,
    required bool nonApplicableBool,
  }) async {
    await initApiData();
  ...
  Future<EvaluationModel?> sendReponseQustionsNote({
    required QuestionModel questionModel,
    required int note,
  }) async {
    await initApiData();
  ...
  Future<EvaluationModel?> sendReponseQustionsDiffererBool({
    required QuestionModel questionModel,
    required bool defererBool,
  }) async {
    await initApiData();
  ...
  Future<EvaluationModel?> sendReponseQustionsImmediateBool({
    required QuestionModel questionModel,
    required bool immdiateBool,
  }) async {
    await initApiData();
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
  ...
  Future<EvaluationModel?> sendReponseQustionsCommentaire({
    required QuestionModel questionModel,
  ...
  Future<EvaluationModel?> sendReponseQustionsDate({
    required QuestionModel questionModel,
  ...
  Future<List<ImageModel>?> sendReponseImage(
      {required QuestionModel questionModel,
      required String imageBytes,
      required String imageType,
      required String imageVign}) async {
    await initApiData();
...
//   Image_DESCRIPTION: , LAST_UPDT: 09/10/2024 16:53:00.000, USER_IDF: 3689, Image_DEFAULT: , USR_LIB: yaakoub}]
  ...
  Future<List<ImageModel>?> sendReponseDeleteImage({
    required QuestionModel questionModel,
    required ImageModel imageModel,
  }) async {
    await initApiData();
    print(
        "delete imageeee: idf ${imageModel.Image_IDF} ,USER_IDF ${questionModel.questionWithoutMemoModel.SVR_IDF} ");
  ...
  Future<String?> sendReponseSignature(
      {required String? LIEINSPSVR_IDF,
      required String imageBytes,
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Lieu,Set_Server_Signature_Eval,Set_Server_Signature_Eval",
          "LIEINSPSVR_IDF": LIEINSPSVR_IDF ?? "12066",
          "LIEINSPSVR_SIGNATURE": imageBytes,
          "LIEINSPSVR_MEMO": commentaire ?? "",
          "Wid": _Wid!
        },
        cookies: _Cookies);
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      String signatureImage =
          data.getElementFromDataList(elementName: "ServerSignature");
  ...
  Future<GbsystemReponseClotureModel?> sendReponseCloseEvaluation({
    required String LIEINSPSVR_IDF,
  }) async {
    await initApiData();
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Lieu,CloseEval,CloseEval",
          "LIEINSPSVR_IDF": LIEINSPSVR_IDF,
          "Wid": _Wid!
        },
        cookies: _Cookies);
    print(
        '"  "OKey": "Lieu,CloseEval,CloseEval","LIEINSPSVR_IDF": ${LIEINSPSVR_IDF},"Wid": ${_Wid}!"');
    if ((data.data["Data"] as List).isNotEmpty &&
        (data.data["Errors"] as List).isEmpty) {
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            ...
            data.data["Data"][i]["ClientData"] != null &&
            (data.data["Data"][i]["ClientData"] as List).isNotEmpty) {
          reponseClotureModel = GbsystemReponseClotureModel.fromJson(
              (data.data["Data"][i]["ClientData"] as List).first);
        }
      }
      await saveCookies(data.cookies!);
  ...
  Future<bool> ManageCatchErrorsServer({
    ...
    if ((responseServer.data["Data"] as List).isEmpty &&
        (responseServer.data["Errors"] as List).isEmpty) {
      await saveCookies(responseServer.cookies!);
  ...
  Future<List<SiteGestionStockModel>?> getAllSiteGestionStock() async {
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "mobile_application,UserDossier_List,UserDossier_List",
          "Wid": _Wid!
        },
        cookies: _Cookies);
    if ((data.data["Data"] as List).isNotEmpty &&
  ...
  Future<List<SalarieGestionStockModel>?> getAllSalarieGestionStock(
      {required SiteGestionStockModel site}) async {
    await initApiData();

    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "mobile_application,DossierServer_List,DossierServer_List",
          "ITEM_IDF": site.DOS_IDF ?? "",
          "Wid": _Wid!
        },
        cookies: _Cookies);
  ...
  Future<List<SalarieGestionStockModel>?> getAllSalarieGestionStockWith_DOS_IDF(
      {required String DOS_IDF}) async {
    await initApiData();
  ...
  Future<ArticleAndDataSetModel?> getAllArticlesSalarieGestionStock(
      {required SalarieGestionStockModel salarie,
      required SiteGestionStockModel site}) async {
    await initApiData();
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "mobile_application,ServerArticle,ServerArticle",
          "ITEM_IDF": salarie.SVR_IDF,
          "ITEM_CODE": site.DOS_IDF ?? "",
          "Wid": _Wid!
        },
        cookies: _Cookies);
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ClientData"] != null) {
          listArticles = ArticleSalarieGestionStockModel
              .convertDynamictoListArticlesSalarie(
                  data.data["Data"][i]["ClientData"]);
        }
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
  ...
  Future<dynamic> getAllArticlesGestionStock() async {
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {"OKey": "mobile_application,Select_Entite,ARTREF", "Wid": _Wid!},
        cookies: _Cookies);
    print(data.data);
    if ((data.data["Data"] as List).isNotEmpty &&
        (data.data["Errors"] as List).isEmpty) {
  ...
  Future<GbsystemArticleGestionStockModel?> affectuerArticleByTag({
    required SalarieGestionStockModel salarie,
    required SiteGestionStockModel site,
    required String tag,
  }) async {
    await initApiData();
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "mobile_application,AffectArticle,AffectArticle",
          "ITEM_IDF": salarie.SVR_IDF,
          "ITEM_CODE": site.DOS_IDF ?? "",
          "ITEM_LIB": tag,
          "Wid": _Wid!
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ArticlesRef_Ds"] != null) {
          listArticles =
              GbsystemArticleGestionStockModel.convertDynamictoListArticles(
                  data.data["Data"][i]["ArticlesRef_Ds"]);
        }
      }
  ...
  Future<GbsystemArticleRefModel?> getArticleByTag({
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          "OKey":
              "mobile_application,READ_ARTREF_INFO_TAG,READ_ARTREF_INFO_TAG",
          "TAG_CODE": tag,
          // "TAG_TYPE": tagType ?? "3",
          "Wid": _Wid!
        },
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ClientData"] != null) {
          listArticles = GbsystemArticleRefModel.convertDynamictoListArticles(
              data.data["Data"][i]["ClientData"]);
        }
      }
  ...
  Future<List<ArticleSalarieGestionStockModel>?> affectuerArticle({
    required SalarieGestionStockModel salarie,
    required SiteGestionStockModel site,
    required GbsystemArticleGestionStockModel article,
  }) async {
    await initApiData();
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          ...
          "ITEM_IDF": salarie.SVR_IDF,
          "ITEM_CODE": site.DOS_IDF ?? "",
          "ITEM_LIB": article.ARTFOUREF_IDF,
          "START_DATE":
              "${DateTime.now().day}/${DateTime.now().month}/${DateTime.now().year}",
          "Wid": _Wid!
        },
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ClientData"] != null) {
          listArticlesSalarie = ArticleSalarieGestionStockModel
              .convertDynamictoListArticlesSalarie(
                  data.data["Data"][i]["ClientData"]);
        }
      }
  ...
  Future<List<ArticleSalarieGestionStockModel>?> retourArticle({
    required SalarieGestionStockModel salarie,
    required SiteGestionStockModel site,
    required ArticleSalarieGestionStockModel article,
  }) async {
    await initApiData();
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "mobile_application,Retour_Stock,Retour_Stock",
          "ITEM_IDF": salarie.SVR_IDF,
          "ITEM_CODE": article.ART_IDF,
          "ITEM_LIB": site.DOS_IDF ?? "",
          "START_DATE":
              "${DateTime.now().day}/${DateTime.now().month}/${DateTime.now().year}",
          "Wid": _Wid!
        },
  ...
  Future<List<QuestionnaireModel>?> getQuestionnaireCauserie(
      {required DateTime dateDebut}) async {
    await initApiData();
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          ...
          "FLIEINSTYP_IDF": "5",
          "Prefix": "LIEINSPQSNR",
          "sidx": "LIEINSPQSNR_CODE",
          "eval_select": "true",
          "START_DATE": "${dateDebut.day}/${dateDebut.month}/${dateDebut.year}",
          "Wid": _Wid!
        },
        cookies: _Cookies);
    print(dateDebut);
  ...
  Future<List<SalarieCuaserieModel>?> getSalariesQuickAccessCauserie(
      {required SiteQuickAccesModel site,
      required QuestionnaireModel questionnaire}) async {
    await initApiData();

    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Lieu,load_servers_for_eval,load_servers_for_eval",
          "LIE_IDF": site.LIE_IDF,
        ...
        cookies: _Cookies);
    print(
        ' "OKey": "Lieu,load_servers_for_eval,load_servers_for_eval","LIE_IDF": ${site.LIE_IDF},"LIEINSPQSNR_IDF": ${questionnaire.LIEINSPQSNR_IDF},"sidx": "SVR_LIB","Wid": _Wid!');
    print("datataa  :: ${data.data}");
    if ((data.data["Data"] as List).isNotEmpty) {
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["list_salaries"] != null) {
          listSalaries =
              SalarieCuaserieModel.convertDynamictoListSalariesDataSet(
                  data.data["Data"][i]["list_salaries"]["rows"]);
        }
      }
      await saveCookies(data.cookies!);
      return listSalaries;
  ...
  Future<String?> getPhotoSalarieCauserie(
      {required String salarieCauserieModel_SVR_IDF}) async {
    await initApiData();
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          ...
          "PREFIX": "SVR",
          "ENTITY_IDF_NAME": "SVR_IDF",
          "ENTITY_IDF_VALUE": salarieCauserieModel_SVR_IDF,
          "Wid": _Wid!
        },
        cookies: _Cookies);
  ...
  Future<List<QuestionTypeModel>?> getAllQuestionTypeCauserie(
      {required String site_LIE_IDF,
      required String questionnaire_LIEINSPQSNR_IDF,
      bool detectLocation = true}) async {
    Position? currentPosition;
    await initApiData();
    if (detectLocation) {
      try {
        ...
        GBSystem_ManageCatchErrors().catchErrors(
          context,
          message: e.toString(),
          method: "getAllQuestionTypeCauserie",
          page: "GBSystem_auth_service",
        );
      }
    }
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          ...
          "LIEINSPSVR_LATITUDE":
              currentPosition?.latitude.toString().replaceAll(".", ",") ?? "0",
          "LIEINSPSVR_LONGITUDE":
              currentPosition?.longitude.toString().replaceAll(".", ",") ?? "0",
          "Wid": _Wid!
        },
        cookies: _Cookies);
    if ((data.data["Data"] as List).isNotEmpty &&
...
//change evaluation cordonner
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["EvaluationStat"] != null) {
          EvaluationModel newEval = EvaluationModel.fromJson(
              data.data["Data"][i]["EvaluationStat"][0]);
  ...
  Future<List<QuestionModel>?> getAllQuestionsOfQuestionTypeCauserie({
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          "OKey":
              "Lieu,Get_Eval_For_Server_By_Categories,Get_Eval_For_Server_By_Categories",
          "LIEINSPCAT_IDF": questionType.LIEINSPCAT_IDF,
  ...
  Future<List<SalarieFormulaireCuaserieModel>?>
      getListSalariesFormulaireCauserie({
    required String? site_LIE_IDF,
    required String? site_CLI_IDF,
    required QuestionnaireModel questionnaire,
    required List<SalarieCuaserieModel> salaries,
    required QuestionTypeModel questionType,
  }) async {
    await initApiData();
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["List_servers"] != null) {
          listSalaries = SalarieFormulaireCuaserieModel
              .convertDynamictoListSalariesFormulaire(
                  data.data["Data"][i]["List_servers"]);
        }
//change evaluation cordonner
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
        ...
        if (data.data["Data"][i] != null &&
          ...
          formulaireController.set_LIEINSPSVR_IDF =
              data.data["Data"][i]["ClientData"][0]["LIEINSPSVR_IDF"];
        }
      }
      await saveCookies(data.cookies!);
      return listSalaries;
  ...
  Future<String?> sendReponseSignatureCauserie(
      {required String? LIEINSPSVR_IDF,
      required String imageBytes,
      required SalarieCuaserieModel salarie,
      required QuestionTypeModel questionType,
      required String? commentaire}) async {
    await initApiData();
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ServerSignature_list"] != null) {
  ...
  Future<List<CauserieModel>?> getAllCauserieTerminer({
    SiteQuickAccesModel? site,
    DateTime? dateDebut,
    DateTime? dateFin,
  }) async {
    await initApiData();
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          ...
          "ITEM_CODE": "1",
          "START_DATE":
              "${selectedDebut.day}/${selectedDebut.month}/${selectedDebut.year}",
          "END_DATE":
              "${selectedFin.day}/${selectedFin.month}/${selectedFin.year}",
          "sidx": "SVR_LIB",
          "FieldsToShow": "-1",
          "Wid": _Wid!
        },
  ...
  Future<bool> deleteCauserie({
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Lieu,LIEINSPSVR,delete",
          "USER_IDF": causerieModel.USER_IDF,
          "LAST_UPDT":
              "${DateTime.now().day}/${DateTime.now().month}/${DateTime.now().year} ${DateTime.now().hour}:${DateTime.now().minute}",
  ...
  Future<List<SalarieFormulaireCuaserieModel>?>
      getAllSalariesCauserieFormulaire(
          {required CauserieModel causerieModel}) async {
    await initApiData();
    final evaluationController =
        Get.put<EvaluationController>(EvaluationController());
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
          ...
          listQustType = SalarieFormulaireCuaserieModel
              .convertDynamictoListSalariesFormulaire(
                  data.data["Data"][i]["List_servers"]);
        }
//change evaluation cordonner
  ...
  Future<String?> getSignatureResponsableCauserie(
      {required CauserieModel causerieModel}) async {
    await initApiData();
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Lieu,Get_Server_Signature_Eval,Get_Server_Signature_Eval",
          "LIEINSPSVR_IDF": causerieModel.LIEINSPSVR_IDF,
          "Wid": _Wid!
        },
        cookies: _Cookies);
  ...
  Future<SignatureSalarieModel?> getSignatureSalarieCauserie(
      {required CauserieModel causerieModel,
      required String salarie_SVR_IDF}) async {
    await initApiData();
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        ...
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["serveur_memo"] != null) {
  ...
  Future<List<GbsystemFournisseurModel>?> getAllFournisseur() async {
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Parameters,fournisseur_int,select",
          "nd": "1725354923783",
          "sidx": "FOU_CODE",
          "sord": "asc",
          "Prefix": "FOU",
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            ...
            data.data["Data"][i]["ClientData"] != null) {
          listFournisseur =
              GbsystemFournisseurModel.convertDynamictoListFournisseur(
                  data.data["Data"][i]["ClientData"]);
        }
      }
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ClientData"] != null) {
          listColors = GbsystemColorModel.convertDynamictoListColor(
              data.data["Data"][i]["ClientData"]["rows"]);
        }
      }
  ...
  Future<List<GbsystemTailleModel>?> getAllTailles(
      {required GbsystemArticleRefModel article}) async {
    await initApiData();

    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "ArticleRef_Manager,charge_Catalog_Fouri,Server_Taille",
          "ITEM_IDF": article.ARTCAT_IDF,
          "Wid": _Wid!
        },
        cookies: _Cookies);
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ClientData"] != null) {
          listColors = GbsystemTailleModel.convertDynamictoListTailles(
              data.data["Data"][i]["ClientData"]);
        }
      }
  ...
  Future<List<GbsystemCatalogueModel>?> AddArticleRefToCatalogue({
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          "OKey":
              "ArticleRef_Manager,charge_Catalog_Fouri,Genere_Catalog_Fouri_NEW",
          // "ARTFOUREF_IDF": "",
          // "ARTFOUREF_UIDF": "",
          "CLR_IDF": color?.CLR_IDF ?? "",
          "FOU_IDF": fournisseur.FOU_IDF,
          "TPOI_IDFS": listTailles
                  ?.map((taille) => taille.TPOI_IDF)
                  .join(",")
                  .toString() ??
              "",
          "ARTFOUREF_TYPE": "0",
          ...
          "ARTFOUREF_PRIX": prix ?? "",
          "ARTFOUREF_START_DATE": dateDebut != null
              ? ConvertDateService().parseDateAndTime(date: dateDebut)
              : "",
          "ARTFOUREF_END_DATE": dateFin != null
              ? ConvertDateService().parseDateAndTime(date: dateFin)
              : "",
          "LAST_UPDT":
              ConvertDateService().parseDateAndTime(date: DateTime.now()),
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["Reconcile"] != null) {
          listColors = GbsystemCatalogueModel.convertDynamictoListCatalogue(
              data.data["Data"][i]["Reconcile"]);
        }
      }
  ...
  Future<bool> deleteCatalogue(
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          "OKey":
              "ArticleRef_Manager,charge_Catalog_Fouri,Delete_Ligne_Catalog",
  ...
  Future<List<GbsystemEnterpotModel>?> getAllEnterpot() async {
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {
          "OKey": "Parameters,ENTR_WithFilter,select",
          "nd": "1725454999973",
          "sidx": "ENTR_CODE",
          "sord": "asc",
          "Prefix": "ENTR",
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ClientData"] != null) {
          listEnterpot = GbsystemEnterpotModel.convertDynamictoListEnterpot(
              data.data["Data"][i]["ClientData"]["rows"]);
        }
      }
  ...
  Future<bool> AddCatalogueAuStock({
    required GbsystemCatalogueModel catalogue,
    required GbsystemEnterpotModel enterpot,
    required DateTime? dateDebut,
    required DateTime? dateFin,
    required String qte,
  }) async {
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          ...
          "ARTFOUREF_IDF": catalogue.ARTFOUREF_IDF,
          "ART_START_DATE": dateDebut != null
              ? ConvertDateService().parseDate(date: dateDebut)
              : "",
          "ART_END_DATE": dateFin != null
              ? ConvertDateService().parseDate(date: dateFin)
              : "",
  ...
  Future<GbsystemCatalogueModel?> updateCatalogue({
    required GbsystemCatalogueModel catalogue,
    required DateTime dateDebut,
    required DateTime? dateFin,
    required String prix,
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          "OKey":
              "ArticleRef_Manager,charge_Catalog_Fouri,Update_Ligne_Catalog",
          "ARTFOUREF_IDF": catalogue.ARTFOUREF_IDF,
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["Reconcile"] != null) {
          catalogueModel = GbsystemCatalogueModel.fromJson(
              data.data["Data"][i]["Reconcile"][0]);
        }
      }
  ...
  Future<List<GbsystemArticleRefModel>?> getAllArticlesRef() async {
    ...
    ResponseModel data = await AppManageApi(context).post(
        url: _ActiveUrl,
        data: {"OKey": "Mobile_Application,ARTREF,Select_Entite", "Wid": _Wid!},
        cookies: _Cookies);
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["ClientData"] != null) {
          listArtRef = GbsystemArticleRefModel.convertDynamictoListArticles(
              data.data["Data"][i]["ClientData"]);
        }
      }
  ...
  Future<List<GbsystemAgenceModel>?> getAllAgences(
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          ...
          "CNX_APPLICATION": GbsSystemServerStrings.bmPlanning_PriseService,
          "CNX_TYPE": 'planner',
          "Count": '0',
          "FieldsToShow": '1',
          "Wid": "0",
  ...
  Future<List<GbsystemArticleGestionStockModel>?>
      getAllArticlesNonAffectuer() async {
    await initApiData();
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          "OKey":
              "ArticleRef_Manager,Charger_Articles_NonAffectes_Salarie,Charger_Articles_NonAffectes_Salarie",
          "ITEM_STOCK_TYPE": "0",
          "Wid": _Wid!
        },
        cookies: _Cookies);
  ...
  //   }
  // }
  Future<List<ArticleAffectuierMultipleGestionStockModel>?>
      affectuerArticleToSalaries({
    required List<SalarieGestionStockModel> listSalariesSelectionner,
    required GbsystemArticleGestionStockModel articleNonAffectuer,
  }) async {
    await initApiData();
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          "OKey":
              "ArticleRef_Manager,Insert_Server_Article,Insert_Server_Article",
          "ARTFOUREF_IDF": articleNonAffectuer.ARTFOUREF_IDF,
          "ENTR_IDF": articleNonAffectuer.ENTR_IDF,
          "STOCK_TYPE": "0",
          "SVR_IDF": listSalariesSelectionner
              .map((taille) => taille.SVR_IDF)
              .join(",")
              .toString(),
          "Wid": _Wid!
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      for (var i = 0; i < (data.data["Data"] as List).length; i++) {
        if (data.data["Data"][i] != null &&
            data.data["Data"][i] is Map &&
            data.data["Data"][i]["Reconcile"] != null) {
          listArtRef = ArticleAffectuierMultipleGestionStockModel
              .convertDynamictoListArticlesSalarie(
                  data.data["Data"][i]["Reconcile"]);
        }
      }
  ...
  Future<PauseModel?> updatePauseVacation({
    required VacationModel vacation,
    required TimeOfDay debutPause,
    required TimeOfDay finPause,
  }) async {
    await initApiData();
    ...
    DateTime finPauseDate = DateTime(DateTime.now().year, DateTime.now().month,
        DateTime.now().day, finPause.hour, finPause.minute);
...
// /////////////////////////////////////
    ...
    if (vacation.VAC_START_HOUR != null && vacation.VAC_END_HOUR != null) {
      isDefferentJourVacation = DateTime(
            DateTime.now().year,
            DateTime.now().month,
            DateTime.now().day,
    ...
    ResponseModel data = await AppManageApi(context).post(
        ...
        data: {
          ...
          "JOB_FILTER_CKBX": "0",
          "VACS_TO_UPDATE": "",
          "VAC_START_HOUR": vacation.VAC_START_HOUR != null
              ? ConvertDateService()
                  .parseDateAndTimeWithoutSec(date: vacation.VAC_START_HOUR!)
              : "",
          "VAC_END_HOUR": vacation.VAC_END_HOUR != null
              ? ConvertDateService()
                  .parseDateAndTimeWithoutSec(date: vacation.VAC_END_HOUR!)
              : "",
          "VAC_BREAK": durationSeconds.toString(),
          "VAC_BREAKSTART_HOUR": ConvertDateService()
              .parseDateAndTimeWithoutSec(date: debutPauseDate),
          "VAC_BREAKEND_HOUR": ConvertDateService().parseDateAndTimeWithoutSec(
              date: isDefferentJour
                  ? finPauseDate.add(Duration(days: 1))
                  : finPauseDate),
          "Wid": _Wid!
        },
        cookies: _Cookies);
    ...
    for (var i = 0; i < (data.data["Data"] as List).length; i++) {
      if (data.data["Data"][i] != null &&
          data.data["Data"][i] is Map &&
          (data.data["Data"][i] as Map).containsKey("SHIFT_CAL_MSG") &&
          data.data["Data"][i]["SHIFT_CAL_MSG"] is List &&
          (data.data["Data"][i]["SHIFT_CAL_MSG"] as List).isNotEmpty) {
        showErrorDialog(
            context, "${data.data["Data"][i]["SHIFT_CAL_MSG"][0]["MESSAGES"]}");
      }
    }
    // showErrorDialog(
    ...
    if ((data.data["Data"] as List).isNotEmpty &&
      ...
      String VAC_BREAKSTART_HOUR =
          data.getFirstElementFromDataList()["VAC_BREAKSTART_HOUR"];
      String VAC_BREAKEND_HOUR =
          data.getFirstElementFromDataList()["VAC_BREAKEND_HOUR"];
      String VAC_DURATION =
          data.getFirstElementFromDataList()["VAC_DURATION_STR"];
      ...
      return PauseModel(
          ...
          VAC_BREAKSTART_HOUR: VAC_BREAKSTART_HOUR,
          VAC_BREAKEND_HOUR: VAC_BREAKEND_HOUR);
    } else {
      return null;
    }
  }
  ...
  Future<DataServerModel?> getSiteS19Client(
    ...
    print(
        "used url to get entreprise : https://192.168.1.120/BMServerR.dll/BMDP81679E6763294DE3827D7D93EEE89436A?d-page=D1BAA8F26A9E418093FB0CDA98846062&SYSMENT_CODE=$clientName&SYSMENT_APPNAME=${ActiveApplication_Params.get_Application_Name_Entreprise()}");
  ...
  Future<bool> verifierExistUrlS19() async {
    ...
    await SharedPreferences.getInstance().then((value) {
      ...
      print("siiiiiiiiiiit :${value.getString(GbsSystemServerStrings.kS19)}");
