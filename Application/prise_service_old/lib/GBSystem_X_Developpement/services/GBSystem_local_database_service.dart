...
// quick access eval
  ...
  static final _box = Hive.box(GbsSystemServerStrings.kHiveBox_Evaluation);

  static Future<void> saveOrUpdateEval({
    ...
    try {
      ...
      final key = _box.keys.firstWhere(
        (k) => _box.get(k)['LIEINSPSVR_IDF'] == LIEINSPSVR_IDF,
        orElse: () => null,
      );
      ...
      if (key != null) {
        ...
        print(
            "Updated existing evaluation_status for IDF: $LIEINSPSVR_IDF == $data");
      } else {
        await _box.add(data);
  ...
  static Map<String, dynamic>? getByIdfEval(String LIEINSPSVR_IDF) {
    try {
      return _box.values.firstWhere(
        (element) => element['LIEINSPSVR_IDF'] == LIEINSPSVR_IDF,
        orElse: () => null,
      );
    } catch (e) {
      return null;
