...
class GBSystemSelectItemSiteGestionStockController extends GetxController {
  ...
  RxList<SiteGestionStockModel> filtredsites =
      RxList<SiteGestionStockModel>([]);
  ...
  Future selectItemVacationFunction(
      {required SiteGestionStockModel selectedVacation}) async {
    siteGestionStockController.setCurrentSiteSite = selectedVacation;
    Get.back();
  }

  Future getListSalarieDependSite(
      {required SiteGestionStockModel selectedSite}) async {
    isLoading.value = true;

    await GBSystem_AuthService(context)
        .getAllSalarieGestionStock(site: selectedSite)
        .then((Salarie) async {
