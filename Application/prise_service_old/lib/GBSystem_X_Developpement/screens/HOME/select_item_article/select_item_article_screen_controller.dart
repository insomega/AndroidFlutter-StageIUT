...
import 'package:gbs_new_project/GBSystem_X_Developpement/screens/HOME/qr_code_page/qr_code_screen.dart';
import 'package:get/get.dart';

class GBSystemSelectItemArticleScreenController extends GetxController {
  GBSystemSelectItemArticleScreenController({
    required this.context,
  });
  BuildContext context;

  RxBool isLoading = RxBool(false);
  RxList<GbsystemArticleRefModel> articles =
      RxList<GbsystemArticleRefModel>([]);
  RxList<GbsystemArticleRefModel> filtredArticles =
      RxList<GbsystemArticleRefModel>([]);
  ...
  void filterDataSite(String query) {
    ...
    filtredArticles.value = articles.where((art) {
      return art.ARTREF_LIB
              .toString()
              .toLowerCase()
              .contains(query.toLowerCase()) ||
          art.ARTCAT_LIB.toString().toLowerCase().contains(query.toLowerCase());
    }).toList();
  }

  void selectItemSiteFunction(
      {required GbsystemArticleRefModel selectedArt}) async {
    articlesController.setSelectedArticle = selectedArt;
    Get.to(QrCodeScreen(
      articleRefModel: selectedArt,
    ));
