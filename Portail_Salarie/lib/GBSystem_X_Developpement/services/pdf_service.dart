...
import 'package:portail_salarie/GBSystem_X_Developpement/services/share_plus_service.dart';
...
class PDFService {
  ...
  Future<double> getAndroidVersion() async {
    ...
    return double.parse(
        androidInfo.version.release); // Extracts Android version
  }

  Future<String?> downloadAndSavePDF(
      String fileName, Uint8List pdfBytes, BuildContext context,
      {bool addExtension = false, bool autoShare = true}) async {
    try {
      ...
      if (autoShare) {
        if (Platform.isAndroid) {
          ...
          if (androidVersion <= 9) {
            ...
            SharePlusService().shareFile(filePath);
          }
        } else {
          SharePlusService().shareFile(filePath);
        }
